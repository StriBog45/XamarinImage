...
{
    public class PressurePage : ContentPage
    {
        //bool start = true;
        ImageSource img;
        ...
        public PressurePage()
        {
            ...
            img = ImageSource.FromUri(new Uri("https://pp.userapi.com/c849032/v849032582/141c68/rlDZQUnv9AQ.jpg"));
        ...
        {
            var path = await DependencyService.Get<IFileWorker>().GetPath("PressureExcel.xlsx");
            var list = await DependencyService.Get<IImageWorker>().PressureDivider(img);
        ...
        private async void ButtonGetData_ClickedAsync(object sender, EventArgs e)
        {
            ...
            {
                entries.Add(new Entry(point.Item1)
                {
                    Color = SKColor.Parse("#FF1493"),
                    ValueLabel = point.Item1.ToString()
                });
                string time = point.Item3.Item2 < 10 ? "0" + point.Item3.Item2 : point.Item3.Item2.ToString();
                listPulse.Add(String.Format("Пульс: {0}//{1} Время: {2}:{3}", point.Item1, point.Item2, point.Item3.Item1, time));

            }
            Content = new StackLayout
            {
                Children = {
                        new ChartView { Chart = new PointChart() { Entries = entries }, VerticalOptions = LayoutOptions.FillAndExpand, HeightRequest = 250  },
